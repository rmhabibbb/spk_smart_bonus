...
{ 
      ...
      $this->data['list_bonus'] = $this->Bonus_m->get_by_order('id_bonus','desc',['status' => 2]);    
      $this->data['index'] = 1;
      $this->data['content'] = 'hrd/dashboard';
...
{   
    if ($this->POST('input')) {
        ...
        foreach ($list_kriteria as $v) { 
            ...
            $d = $this->DSubUmum_m->get_row(['id_sub_umum' => $this->POST('kriteria_'.$v->id_kriteria_umum)]);
             

            $data = [
              'id_kriteria_umum' => $v->id_kriteria_umum,
              'id_bonus' => $id_bonus,
              'nip' => $nip, 
              'id_sub_umum' => $this->POST('kriteria_'.$v->id_kriteria_umum),
              'nilai' =>$d->nilai,
              'keterangan' => $d->keterangan 
            ];
            $this->PenilaianHRD_m->insert($data); 
        }

         $this->flashmsg('NILAI BERHASIL DIINPUT!', 'success');
          redirect('hrd/penilaian/'.$id_bonus);
          exit(); 
    }
    ...
    if ($this->POST('edit')) {
        ...
        $id_bonus = $this->POST('id_bonus');
        $nip = $this->POST('nip');
        $this->PenilaianHRD_m->delete_by(['id_bonus' => $id_bonus,'nip' => $nip]);
    ...
    if ($this->POST('selesai')) {  
      ...
      redirect('hrd/bonus/'.$this->POST('id_bonus'));
      exit();    
    } 
    if ($this->uri->segment(3) != 'hasildivisi' && $this->uri->segment(3) != '') {
      ...
      if ($this->data['bonus']->status == 3) {
        redirect('hrd/bonus/'.$this->data['bonus']->id_bonus);
        exit();
      }
      ...
      $this->data['index'] = 1;
      $this->data['content'] = 'hrd/inputnilai';
      $this->template($this->data,'hrd');
        ...
        if ($this->data['bonus']->status == 3) {
          redirect('hrd/bonus/hasildivisi'.$this->data['bonus']->id_bonus);
          exit();
        }
        ...
        $this->data['index'] = 1;
        $this->data['content'] = 'hrd/inputbonus-hasildivisi';
        $this->template($this->data,'hrd');
      } else{
      redirect('hrd/');
      exit();    
...
public function bonus(){
    if ($this->uri->segment(3) != 'hasildivisi' && $this->uri->segment(3) != 'hasilhrd' && $this->uri->segment(3) != '') {
        ...
        $this->data['index'] = 2;
        $this->data['content'] = 'hrd/detailbonus';
        $this->template($this->data,'hrd');
      } 
      elseif ($this->uri->segment(3) == 'hasildivisi') {
        if ($this->Bonus_m->get_num_row(['id_bonus' => $this->uri->segment(4)]) == 0) {
